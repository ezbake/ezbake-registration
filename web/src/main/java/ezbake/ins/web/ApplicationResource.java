...
import ezbake.services.classification.thrift.ClassificationService;
...
public class ApplicationResource extends RegistrationResourceBase {
    ...
    private static final String DEPLOY_PACKAGE = "DEPLOY_PACKAGE";
...
//            Right now we're assuming there's only 1 manifest in the file
            ...
            if (adminRequired) {
                ...
                if (appTarGzFile == null) {
                    ...
                    deploymentTypePackage = new DeploymentTypePackage(DEPLOY_PACKAGE, appArchiveFile, appArchiveFileName, appManifestFile, appPropertiesFiles);
                } else {
                    deploymentTypePackage = new DeploymentTypePackage(DEPLOY_SERVICE, manifests.get(0), ByteBuffer.wrap(Files.toByteArray(appTarGzFile)));
                }
            }
    ...
    @GET
    @Path("/capco")
    public String getAllCapcoData(@Context ContextResolver<ServiceClient> resource) throws Exception {
        ClassificationService.Client client = null;
        ServiceClient serviceClient = getFromContext(resource);
        try {
            client = getClassificationServiceClient(serviceClient);
            return client.GetAllCapcoData("");
        } catch(Exception ex) {
            logger.error("Failed to get capco data.  Only unclassified may be used until Capco service is restored", ex);
            client = invalidateServiceClient(client, serviceClient);
            return "{\"classifications\": [{\"groups\":[{\"markingGroup\":\"US_CLASSIFICATION\",\"markings\":[{\"portion\":\"U\",\"title\":\"UNCLASSIFIED\", \"classifRank\":\"1\"}]},{\"markingGroup\":\"SCI_CONTROLS\",\"markings\":[]},{\"markingGroup\":\"DISSEM_CONTROLS\",\"markings\":[]},{\"markingGroup\":\"NON_IC_CONTROLS\",\"markings\":[]}]}], \"countryData\": [{\"countryName\":\"United States\",\"countryDigraph\":\"US\",\"countryTrigraph\":\"USA\"}], \"orgData\":[{\"orgTetragraph\": \"FVEY\", \"orgTrigraphs\": [\"USA\"]}]}";
        } finally {
        	freeServiceClient(client, serviceClient);
        }
    }
     ...
     */
    public void checkAllowedToRegister(@Context ContextResolver<ServiceClient> resource) throws TException, EzSecurityTokenException {
    	ServiceClient serviceClient = getFromContext(resource);
    		...
    		public String call() throws Exception {
    			...
    			try {
    				switch(deploymentType) {
    					case STAGE_SERVICE:
    						client.stageServiceDeployment(deploymentTypePackage.getArtifactManifest(), deploymentTypePackage.getArtifact(), token);
    						break;
    					case DEPLOY_SERVICE:
    						client.deployService(deploymentTypePackage.getArtifactManifest(), deploymentTypePackage.getArtifact(), token);
    						break;
    					case DEPLOY_PACKAGE:
    	                    PackageDeployer.deployPackage(client, deploymentTypePackage.getPackageFile(), deploymentTypePackage.getPackageFileName(), 
    													  deploymentTypePackage.getManifestFile(),deploymentTypePackage.getConfigFiles(), 
    													  null, token);
    	                    break;
    				}
    ...
    private class DeploymentTypePackage {
    	...
    	private Map<File, String> configFiles;
    	...
    	// constructor for package deployer
    	public DeploymentTypePackage(String aDeploymentType, File aPackageFile, String aPackageFileName, File aManifestFile, Map<File, String> aConfigFiles) {
    		deploymentType = aDeploymentType;
    		packageFile = aPackageFile;
    		packageFileName = aPackageFileName;
    		manifestFile = aManifestFile;
		...
		public Map<File, String> getConfigFiles() {
			return configFiles;
		}
    }
