...
import ezbake.security.thrift.PermissionDeniedException;
...
public class InternalNameServiceHandler extends EzBakeBaseThriftService implements InternalNameService.Iface {
    ...
    private static final String AppNameField = "Application.appName";
	...
	public Set<Application> getDuplicateAppNames(String appName, EzSecurityToken token) throws TException {
        validateToken(token);
        devModeUnsupported();
		return getApplications(String.format("%s.\"%s\"", AppNameField, appName), getSecurityToken());
	}
    ...
    @Override
    public Set<String> getTopicsForFeed(String appId, String feedName) throws ApplicationNotFoundException, TException {
        if (inDevMode) {
    ...
    @Override
    public Set<String> getApprovedTopicsForFeed(String appId, String feedName, EzSecurityToken token) throws ApplicationNotFoundException, TException {
        devModeUnsupported();
        validateToken(token);
    ...
    @Override
    public Set<String> getListeningTopicsForFeed(String appId, String feedName, EzSecurityToken token) throws ApplicationNotFoundException, TException {
    ...
    @Override
    public String getURIPrefix(String appId, String categoryKey) throws ApplicationNotFoundException, TException {
    ...
    @Override
    public Application getAppById(String appId, EzSecurityToken ezSecurityToken) throws ApplicationNotFoundException, TException {
        checkNotNull(ezSecurityToken);
        validateToken(ezSecurityToken);
    ...
    @Override
    public ApplicationSummary getAppByName(String appName, EzSecurityToken token) throws EzSecurityTokenException, ApplicationNotFoundException, TException {
        Set<Application> applications = getApplications(String.format("%s.\"%s\"", AppNameField, appName), token);
        if (applications.size() == 0) {
            throw new ApplicationNotFoundException(appName + " was not found");
    ...
    @Override
    public String exportApplication(String appId, EzSecurityToken securityToken) throws ApplicationNotFoundException, TException {
	...
	@Override
	public Set<JobRegistration> getJobRegistrations(final String appId, final EzSecurityToken ezSecurityToken) throws ApplicationNotFoundException, TException {
		if (inDevMode) {
    ...
    public TProcessor getThriftProcessor() {
        ...
        if (!inDevMode) {
            ...
            deployerSecurityId = pool.getSecurityId(EzDeployServiceConstants.SERVICE_NAME);
    ...
    @VisibleForTesting
    protected Application getApplication(String appId) throws ApplicationNotFoundException, TException{
        return getApplication(appId, getSecurityToken());
    }

    @VisibleForTesting
    protected Application getApplication(String appId, EzSecurityToken token) throws ApplicationNotFoundException, TException {
    ...
    private void devModeUnsupported() {
        if (inDevMode) {
            logger.error("This method is not supported in dev mode");
            throw new IllegalStateException("INS Development Mode does not support this method call");
        }
    }
    ...
    private boolean isApplicationRegistrationApproved(String appId, EzSecurityToken token) throws TException, RegistrationException, SecurityIDNotFoundException, PermissionDeniedException {
    ...
    @VisibleForTesting
	protected RegistrationStatus getApplicationRegistrationStatus(String appId, EzSecurityToken token) throws TException, RegistrationException, SecurityIDNotFoundException, PermissionDeniedException {
    ...
    protected EzSecurityToken getRegistrationSecurityToken(EzSecurityToken origToken) {
        try {
            EzSecurityToken token = securityClient.fetchDerivedTokenForApp(origToken, pool.getSecurityId(
